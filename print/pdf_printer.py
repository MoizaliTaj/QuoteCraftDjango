...
def printer_invoice(invoice_data, item_data, footer_print_date_time, pdf_file_name, print_type):
    #printer_invoice(invoice_data, item_data, printed_on, f"{page_title}.pdf", print_type)
    pdf = PDF()
    pdf.setup_object(invoice_data, item_data, footer_print_date_time, pdf_file_name, print_type)
    pdf.add_page()
    pdf.table()
    pdf_file_name = base + 'public/static/pdf_files/' + pdf_file_name
...
class PDF(FPDF):
    ...
    def setup_object(self, invoice_data, item_data, footer_print_date_time, pdf_file_name, print_type):
        self.print_type = print_type
        self.salesman_data = invoice_data.customer_full_name.salesman_name
        self.customer_data = invoice_data.customer_full_name
        self.invoice_details = invoice_data
        date_print = str(self.invoice_details.date)
        ...
        if print_type != 'without_quantity':
            self.table_header_list = ['image_path', 'Sr.', 'Code', 'Description', 'Quantity', 'Unit', 'Unit Price',
                                      'Total', ]
        else:
        ...
        for row in item_data:
            ...
            if print_type != 'without_quantity':
                if len(row.size):
                    data.append([row.image, counter, remove_problem_character(row.code),
                                 remove_problem_character(row.description + ", " + row.size),
                                 number_formatter(row.quantity), remove_problem_character(row.unit),
                                 number_formatter(row.selling_rate), number_formatter(row.total), ])
                else:
                    data.append([row.image, counter, remove_problem_character(row.code),
                                 remove_problem_character(row.description), number_formatter(row.quantity),
                                 remove_problem_character(row.unit), number_formatter(row.selling_rate),
                                 number_formatter(row.total), ])
            else:
                if len(row.size):
                    data.append([row.image, counter, remove_problem_character(row.code),
                                 remove_problem_character(row.description + ", " + row.size),
                    ...
                    data.append([row.image, counter, remove_problem_character(row.code),
                                 remove_problem_character(row.description), remove_problem_character(row.unit),
                                 number_formatter(row.selling_rate), ])
        ...
        self.bottom_blank_width = self.width_details[1] + self.width_details[2] + self.width_details[3] + \
                                  self.width_details[4]
    ...
    def header_and_customer_details_printer(self):
        self.image(base + 'public/static/media/header.png', x=6.5, y=5, w=self.w - 13)
        self.set_y(34)
        self.set_text_color(0, 0, 0)
        self.set_font(self.font, '', 10.5)
        self.set_x((self.w - self.get_string_width(
            'Mob: ' + self.salesman_data.mobile_number + ' | ' + 'Tel: ' + self.salesman_data.land_line_number + ' | ' + 'Email: ' + self.salesman_data.email + ' | ' + 'TRN 100000000000001')) / 2)
        self.cell(self.get_string_width('Mob: ' + self.salesman_data.mobile_number), 0,
                  'Mob: ' + self.salesman_data.mobile_number, 0, 0, 'C',
                  link="tel: " + self.salesman_data.mobile_number)
        self.cell(self.get_string_width(' | '), 0, ' | ', 0, 0, 'C')
        self.cell(self.get_string_width('Tel: ' + self.salesman_data.land_line_number), 0,
                  'Tel: ' + self.salesman_data.land_line_number, 0, 0, 'C', link="tel: " + self.salesman_data.land_line_number)
        self.cell(self.get_string_width(' | '), 0, ' | ', 0, 0, 'C')
        self.cell(self.get_string_width('Email: ' + self.salesman_data.email), 0,
                  'Email: ' + self.salesman_data.email, 0, 0, 'C', link="mailto: " + self.salesman_data.email)
        self.cell(self.get_string_width(' | '), 0, ' | ', 0, 0, 'C')
        ...
        self.set_x(0)
        self.cell(self.w - 6.5, 0, 'Date: ' + str(self.date_print), 0, 0, 'R')
        self.ln(4.5)
        self.set_x(0)
        self.cell(self.w - 6.5, 0, 'Reference # ' + str(self.invoice_details.id), 0, 0, 'R')
        self.ln(4.0)
        self.set_x(6.5)
        self.set_font(self.font, '', 10)
        self.cell(self.get_string_width('Customer Name: '), 0, 'Customer Name: ', 0, 0, 'L')
        self.set_font(self.font, 'B', 10)
        self.cell(self.get_string_width(self.customer_data.full_name + " "), 0, self.customer_data.full_name, 0,
                  0, 'L')
    ...
    def print_total_amount_details(self, cell_height, font_size):
        ...
        if self.print_type != 'without_vat':
            ...
            self.set_font(self.font, "", font_size)
            self.set_text_color(0, 0, 0)
            self.cell(self.width_details[7], cell_height, number_formatter(self.total_with_vat), border=1, align='R',
                      fill=False)
    ...
    def table(self):
        ...
        for row in self.data:
            ...
            if row[0]:
                image = Image.open(base + 'public/static/' + str(row[0]))
            ...
            if row[0]:
                ...
                if width_change_percent > height_change_percent:
                    self.image(base + 'static/' + row[0], w=image_max_width - 1,
                               x=self.width_details[1] + self.width_details[2] + 6.5 + 0.2, y=current_row_y + 0.4)
                    image_max_height = calculate_revised_size(width, height, image_max_width)
                else:
                    self.image(base + 'public/static/' + str(row[0]), h=image_max_height,
                               x=self.width_details[1] + self.width_details[2] + 6.5 + 0.2, y=current_row_y + 0.4)
    ...
    def footer(self):
        ...
        self.set_x(6.5)
        self.set_font(self.font, '', 8)
        self.cell(self.w - 13, 0, 'Salesman Name: ' + self.salesman_data.full_name, 0, 0, 'L')
        self.ln(3.5)
        self.set_x(6.5)
        self.set_font(self.font, '', 7)
        self.cell(self.w - 13, 0,
                  'Printed on ' + self.footer_print_date_time + ' | All the above prices are in UAE Dirhams. | Above quoted items are subject to prior sales. |',
                  0, 0, 'L')
